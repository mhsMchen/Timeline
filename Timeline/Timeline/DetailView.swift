...
struct DetailView: View {
    ...
    // recalling from sign in view, it will remember the user yet again
    @State var tempNoPass: Bool = true
    @State private var isPresentingPassView2 = false {
        willSet {
            if newValue && tempNoPass == false {
                if isPresentingEditView == true { tempNoPass = false }
            }
        }
    }
    ...
    var body: some View {
        List {
            ...
            VStack {
                    ...
                    .cornerRadius(10)
                    .background(Image("clubBg")).cornerRadius(10)
        ...
        .toolbar {
            Button("Edit") {
                if tempNoPass { isPresentingPassView2 = true } else { isPresentingEditView = true }
                data = club.data
            }
        }
        
        .sheet(isPresented: $isPresentingPassView2) {
            NavigationView {
                passView(data: $data, isPassViewSet: $isPresentingPassView2, isNewClubViewSet: $isPresentingEditView, noPass: $tempNoPass)
                // add or dismiss on top
                    .toolbar {
                        ToolbarItem(placement: .cancellationAction) {
                            Button("Cancel") {
                                isPresentingPassView2 = false
                            }
                        }
                    }
            }
